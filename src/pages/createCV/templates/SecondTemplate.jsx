...
const SecondTemplate = ({
    ...
    const SectionHeading = ({ children }) => (
        <div className="mb-2">
            <h2 className="text-sm font-bold text-black">{children}</h2>
    ...
    const ContactRow = ({ icon, text }) => (
        <div className="flex items-center  gap-2 ">
            <span className="material-symbols-outlined text-[13px]" style={{ color: selectedTheme.primary }}>{icon}</span>
            <span className="text-xs text-gray-700">{text}</span>
        </div>
    );

    const Header = ({ data }) => (
        <div className="px-10 pt-10 pb-4 flex justify-between items-start eleventhTempFont">
            <div>
                <h1 className="text-3xl font-extrabold text-black">
                    {data?.firstName || "Your"} {data?.lastName || "Name"}
                </h1>
                {data?.role && <p className="text-sm font-semibold text-black mt-1">{data.role}</p>}
            </div>
            <div className="space-y-1 shrink-0">
                {(data?.city || data?.country) && <ContactRow icon="location_on" text={[data.city, data.country].filter(Boolean).join(", ")} />}
                {data?.phone && <ContactRow icon="call" text={data.phone} />}
                {data?.email && <ContactRow icon="mail" text={data.email} />}
                {data?.linkedInUrl && <ContactRow icon="link" text={data.linkedInUrl} />}
            </div>
        </div>
    );

    const Summary = ({ data }) => (
        <div className="px-10 py-2 eleventhTempFont">
    ...
    const Skills = ({ data }) => (
        <div className="px-10 py-2 eleventhTempFont">
            ...
            <p className="text-xs text-gray-700 leading-relaxed">{data.map(s => s.skill || s).join(", ")}</p>
        </div>
    );
    ...
    const ProjectItem = ({ data, index }) => (
        <div className="px-10 py-2 eleventhTempFont">
            ...
            <div className="flex justify-between items-baseline">
                ...
                {data.startDate && <p className="text-xs italic text-gray-600">{data.startDate} – {data.endDate || "Present"}</p>}
            </div>
    ...
    const ExperienceItem = ({ data, index }) => (
        <div className="px-10 py-2 eleventhTempFont">
            {(experience.indexOf(data) === 0 || index === 0) && <SectionHeading>Professional Experience</SectionHeading>}
            <div className="flex justify-between items-baseline">
                <p className="text-xs font-bold text-black">{data.jobTitle}</p>
                <p className="text-xs italic text-gray-600 shrink-0 ml-2">
    ...
    const CertificationItem = ({ data, index }) => (
        <div className="px-10 py-2 eleventhTempFont">
            ...
            <div className="flex justify-between items-baseline">
                ...
                {data.issueingDate && <p className="text-xs italic text-gray-600">{data.issueingDate}{data.expirationDate ? ` – ${data.expirationDate}` : ""}</p>}
            </div>
    ...
    const AwardItem = ({ data, index }) => (
        <div className="px-10 py-2 eleventhTempFont">
            ...
            <div className="flex justify-between items-baseline">
                ...
                {data.issueingDate && <p className="text-xs italic text-gray-600">{data.issueingDate}</p>}
            </div>
    ...
    const EducationItem = ({ data, index }) => (
        <div className="px-10 py-2 eleventhTempFont">
            ...
            <div className="">
                {data.country&&<p className="text-xs text-gray-700">
