...
const SixthTemplate = ({ personalInfo, professionalSummary, skills, experience, projects, awards, certifications, education, languages, theme, onPageCount, currentPage }) => {
    ...
    const HEADER_HEIGHT = 195; // photo-box height on page 1 (also used for the name-only header when photo is off)
    const PAGE_1_MAIN = 800;
    const PAGE_N_MAIN = PAGE_HEIGHT - 48; // py-6 top+bottom padding on main column for pages after the first

    const showPhoto = personalInfo?.showPhoto !== false; // default to true unless explicitly unchecked
    ...
    ], [experience, projects, awards, certifications, skills, languages, education]);
    ...
    // ─── Header row: gray photo box (left) + white name/title area (right) — only used when showPhoto is true ───
    ...
    // ─── Name-only header — used inside the main column when showPhoto is false, keeps the same height as HeaderRow so pagination math stays correct ───
    const NameHeader = ({ data }) => (
        <div style={{ height: HEADER_HEIGHT }} className="flex flex-col justify-center sixteenTempFont">
            <h1 className="text-3xl font-extrabold text-black leading-tight">
                {data?.firstName?.toUpperCase() || "YOUR"} {data?.lastName?.toUpperCase() || "NAME"}
            </h1>
            {data?.role && <p className="text-lg font-bold text-black mt-1">{data.role?.toUpperCase()}</p>}
    ...
    // ─── Sidebar: solid black. Runs full page height always — starts at the very top when there's no photo header above it ───
                ...
                <div className="grid grid-cols-2 gap-x-10 gap-y-4">
                    {[left, right].map((col, ci) => (
                        <div key={ci} className="space-y-4">
                            {col.map((l, i) => (
                                <div key={i}>
                                    <p className="text-xs text-black mb-1">{l.language}</p>
            ...
            {pagesToRender.map((page, idx) => {
                ...
                return (
                    <div key={pageIndex} style={{ width: PAGE_WIDTH, height: PAGE_HEIGHT, backgroundColor: "#fff", boxShadow: '0 4px 10px rgba(0,0,0,0.15)', display: "flex", flexDirection: "column" }}>
                        {showPhoto && isFirst && <HeaderRow data={personalInfo} />}
                        <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
                            ...
                            <div style={{ width: MAIN_WIDTH }} className="px-8 py-6 sixteenTempFont">
                                {!showPhoto && isFirst && <NameHeader data={personalInfo} />}
